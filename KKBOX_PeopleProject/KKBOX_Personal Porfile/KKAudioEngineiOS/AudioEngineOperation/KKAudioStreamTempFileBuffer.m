...
- (void)preloadWithPackets:(KKAudioPacketInfo *)inPackets
{
	...
	for (size_t i = 0; i < MAX_PRELOAD_PACKET_COUNT; ++i) {
		packetInfos[i] = inPackets[self.startIndex + i];
		packetInfos[i].packetDescription.mStartOffset = tmpBufferSize;
		tmpBufferSize += packetInfos[i].packetDescription.mDataByteSize;
	}
	dispatch_async(preloadQueue, ^{
		long filePos = (long)packetInfos[0].data;
		packetInfos[0].data = malloc(tmpBufferSize);
		fseek(fileHandle, filePos, SEEK_SET);
		fread(packetInfos[0].data, tmpBufferSize, 1, fileHandle);

		for (size_t i = 1; i < MAX_PRELOAD_PACKET_COUNT; ++i) {
			packetInfos[i].data = packetInfos[0].data + packetInfos[i].packetDescription.mStartOffset;
		}

		_preloadStatus = KKAudioPreloadStatusLoaded;
	});
}
