...
{
	UInt32 busCount = self.busCount;
	for (UInt32 i = 0; i < busCount; i++) {
		__unused OSStatus status = noErr;
		status = AudioUnitSetProperty(self.audioUnit, kAudioUnitProperty_StreamFormat, kAudioUnitScope_Input, i, &destFormat, sizeof(destFormat));
...
- (UInt32)busCount
{
	...
	AudioUnitGetProperty(self.audioUnit, kAudioUnitProperty_ElementCount, kAudioUnitScope_Input, 0, &property, &propertySize);
	return property;
}
